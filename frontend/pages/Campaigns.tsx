...
const Campaigns: React.FC = () => {
  ...
  useEffect(() => {
      ...
      if (saved && selectedSegment) {
          const contacts: Contact[] = JSON.parse(saved);
          // Contar ativos e bloqueados no segmento
          const segmentContacts = contacts.filter(c => c.tags.includes(selectedSegment));
          // Importante: status pode ser undefined em dados antigos, tratar como UNKNOWN
      ...
      return new Promise((resolve, reject) => {
          ...
          reader.onload = (event) => {
              ...
              img.onload = () => {
                  ...
                  const MAX_WIDTH = 1024; // HD max width
                  ...
                  // Export as JPEG with 70% quality (High compression)
  ...
  // Helper to get delay based on mode
  const getDelay = () => {
      switch (delayMode) {
          case 'FAST': return Math.floor(Math.random() * (8000 - 3000) + 3000); // 3s - 8s
          case 'MEDIUM': return Math.floor(Math.random() * (20000 - 10000) + 10000); // 10s - 20s
          case 'SLOW': return Math.floor(Math.random() * (45000 - 25000) + 25000); // 25s - 45s
  ...
  const handleStartCampaign = async () => {
      ...
      // 0. Verificar Limite Diário
      const canSendCheck = campaignControl.canSend();
      if (!canSendCheck.allowed) {
          alert(`❌ ${canSendCheck.reason}\n\n✅ Tente novamente amanhã ou aumente o limite em Configurações.`);
          return;
      }
      
      // 1. Validações
      if (!message && banners.length === 0) { alert("Digite uma mensagem ou adicione banners."); return; }
      if (!selectedSegment) { alert("Selecione um segmento."); return; }
      
      // 2. Buscar Conta Conectada
      ...
      // Procura qualquer conta com status connected
      ...
      // 3. Preparar Contatos (FILTRANDO BLOQUEADOS)
      ...
      // Filtra por segmento E remove bloqueados (ContactStatus.BLOCKED)
      // Garantir que status undefined seja tratado como válido
      const targetContacts = allContacts.filter(c => 
          c.tags.includes(selectedSegment) && c.status !== ContactStatus.BLOCKED
      );

      if (targetContacts.length === 0) {
          if (allContacts.filter(c => c.tags.includes(selectedSegment) && c.status === ContactStatus.BLOCKED).length > 0) {
              alert("Todos os contatos deste segmento estão na Blacklist (Bloqueados). O envio foi abortado.");
          } else {
      ...
      // 4. Loop de Envio (Por Contato)
      for (let i = 0; i < targetContacts.length; i++) {
          const contact = targetContacts[i];
          setCurrentContactName(contact.name);

          // Montar Mensagem Principal (Texto + CTA)
          let mainMessage = message;
          if (ctaText && ctaLink) {
              mainMessage += `\n\n${ctaText}: ${ctaLink}`;
          }
          
          // Mensagem de Descadastre (Separada)
            ...
            if (contactSuccess && banners.length > 0) {
                for (const banner of banners) {
                    ...
                    if (!imgResult.success) {
                        contactSuccess = false;
                        contactError = imgResult.error;
                        break; 
                    }
                    if (banners.length > 1) await new Promise(resolve => setTimeout(resolve, 1000));
                }
            }

            // --- ETAPA 3: Enviar DESCADASTRE (Se ativado, no final) ---
            if (contactSuccess && unsubscribe) {
                // Delay para garantir que fique abaixo da imagem
          ...
          if (contactSuccess) {
              ...
              if (contactError && contactError.includes("client-token is not configured")) {
          ...
          if (!canContinue.allowed) {
              alert(`✅ Campanha pausada!\n\n${canContinue.reason}\n\nEnviadas: ${progress.sent + 1}\nRestantes: ${targetContacts.length - (i + 1)}`);
          ...
          if (campaignControl.shouldPause(messagesSinceLastPause) && i < targetContacts.length - 1) {
              ...
              console.log(`⏸️ PAUSA AUTOMÁTICA: ${settings.pauseDuration} minutos após ${messagesSinceLastPause} mensagens`);
          ...
          // Delay Antibloqueio (se não for o último contato)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-card border border-gray-700 rounded-xl p-4">
          ...
          <div className="text-xs text-gray-500 mt-1">
            {campaignControl.canSend().remaining} restantes
          </div>
        </div>
        ...
        <div className="bg-card border border-gray-700 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-400">Falhas Hoje</span>
          ...
          <div className="text-xs text-gray-500 mt-1">
            {stats.todayFailed > 0 ? 'Verificar conexão' : 'Tudo OK'}
          </div>
        </div>
      </div>
      ...
      {showSettings && (
        <div className="bg-card border border-gray-700 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-bold text-white">⚙️ Configurações de Proteção</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {ACCOUNT_AGE_INFO[settings.accountAge].description}
              </p>
            </div>

            {/* Limite Diário Manual */}
            <div>
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                Sistema pausará automaticamente
              </p>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-card border border-gray-700 rounded-2xl p-6">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    >
                        {availableSegments.length === 0 && <option value="">Nenhum segmento encontrado</option>}
                    ...
                    <div className="flex items-center justify-between mt-1">
                         <p className="text-xs text-green-400 flex items-center">
                            <Users size={12} className="mr-1"/> {contactsCount} aptos
                        </p>
                        {blockedCount > 0 && (
                            <p className="text-xs text-red-400 flex items-center font-bold" title="Estes contatos estão na Blacklist e não receberão mensagens">
                                <ShieldBan size={12} className="mr-1"/> {blockedCount} bloqueados (serão ignorados)
                            </p>
                        )}
                    </div>
                </div>
          ...
          <div className="bg-card border border-gray-700 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                {banners.length < 4 && (
                    <label className={`relative ${aspectClass} border-2 border-dashed border-gray-600 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-primary hover:bg-gray-800 transition-all text-gray-500 hover:text-primary ${campaignStatus === 'SENDING' ? 'opacity-50 pointer-events-none' : ''}`}>
                        {isProcessingImage ? (
                            <Loader2 className="animate-spin mb-2" size={24}/>
                        ) : (
                            <ImageIcon size={24} className="mb-2" />
                        )}
                        <span className="text-xs text-center">{isProcessingImage ? 'Otimizando...' : 'Adicionar Imagem'}</span>
                        <span className="text-[10px] opacity-70">{isProcessingImage ? 'Aguarde' : `(${aspectRatio})`}</span>
                        <input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} disabled={campaignStatus === 'SENDING' || isProcessingImage} />
                    </label>
                )}
            </div>
             ...
             <div className="mt-6 space-y-4 border-t border-gray-700 pt-6">
                ...
                <div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg">
                    ...
                    <button 
                        ...
                        disabled={campaignStatus === 'SENDING'}
                        className={`w-12 h-6 rounded-full relative transition-colors ${unsubscribe ? 'bg-primary' : 'bg-gray-600'}`}
                    >
                        <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${unsubscribe ? 'left-7' : 'left-1'}`}></div>
                    </button>
                </div>
             </div>
          </div>
            ...
            {campaignStatus !== 'IDLE' && campaignStatus !== 'PAUSED' && (
                <div className="bg-card border border-gray-700 rounded-2xl p-6 shadow-xl animate-in slide-in-from-bottom">
                    <h3 className="text-lg font-bold text-white mb-4 flex items-center">
                        {campaignStatus === 'SENDING' ? <Loader2 className="animate-spin mr-2 text-primary" /> : <CheckCircle2 className="mr-2 text-green-500" />}
                ...
                <div className="bg-[#0b141a] border-[8px] border-gray-800 rounded-[3rem] h-[600px] overflow-hidden relative shadow-2xl">
                    ...
                    >
                        ...
                        <div className="bg-[#202c33] rounded-lg p-2 max-w-[90%] ml-auto mr-0 shadow-sm mb-2">
                            <p className="text-white text-sm whitespace-pre-wrap">{message || "Sua mensagem aparecerá aqui..."}</p>
                ...
                {campaignStatus === 'IDLE' && (
                    <div className="mt-6 space-y-3">
                        ...
                        <div className="flex items-center justify-center text-xs text-gray-500 space-x-1">
                            <Clock size={12} />
                            <span>Tempo estimado: {Math.ceil((contactsCount * (delayMode === 'FAST' ? 5 : delayMode === 'MEDIUM' ? 15 : 35)) / 60)} min</span>
                        </div>
                    </div>
                )}
            </div>
